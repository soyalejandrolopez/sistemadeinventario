...
<div class="container">
    <div class="row mb-4">
        ...
        <div class="col-md-4 text-end">
            <a href="{{ route('ordenes.show', $orden->id) }}" class="btn btn-secondary">
    ...
    <div class="card">
        <div class="card-body">
            <form action="{{ route('ordenes.update', $orden->id) }}" method="POST" id="ordenForm">
                @csrf
                @method('PUT')
                ...
                <div class="row mb-4">
                        ...
                        <div class="mb-3">
                            ...
                            <select class="form-select @error('proveedor_id') is-invalid @enderror" id="proveedor_id" name="proveedor_id" required>
                                ...
                                @foreach($proveedores as $id => $nombre)
                                    <option value="{{ $id }}" {{ old('proveedor_id', $orden->proveedor_id) == $id ? 'selected' : '' }}>{{ $nombre }}</option>
                                @endforeach
                            </select>
                            @error('proveedor_id')
                    ...
                    <div class="col-md-3">
                        <div class="mb-3">
                            ...
                            <input type="date" class="form-control @error('fecha_orden') is-invalid @enderror" id="fecha_orden" name="fecha_orden" value="{{ old('fecha_orden', $orden->fecha_orden->format('Y-m-d')) }}" required>
                            @error('fecha_orden')
                            <div class="invalid-feedback">
                                {{ $message }}
                            </div>
                    ...
                    <div class="col-md-3">
                        <div class="mb-3">
                            ...
                            <input type="date" class="form-control @error('fecha_entrega') is-invalid @enderror" id="fecha_entrega" name="fecha_entrega" value="{{ old('fecha_entrega', $orden->fecha_entrega->format('Y-m-d')) }}" required>
                            @error('fecha_entrega')
                            <div class="invalid-feedback">
                                {{ $message }}
                            </div>
                ...
                <div class="mb-3">
                    ...
                    <textarea class="form-control @error('observaciones') is-invalid @enderror" id="observaciones" name="observaciones" rows="2">{{ old('observaciones', $orden->observaciones) }}</textarea>
                    @error('observaciones')
                    <div class="invalid-feedback">
                        {{ $message }}
                    </div>
                ...
                <div class="table-responsive mb-3">
                    <table class="table table-striped" id="productos_tabla">
                        ...
                        <tbody>
                            <!-- Filas de productos se cargarán dinámicamente -->
                        </tbody>
                        <tfoot>
                            <tr>
                                <td colspan="4" class="text-end"><strong>Total:</strong></td>
                                <td><strong id="total_orden">${{ number_format($orden->total, 2) }}</strong></td>
                                <td></td>
                            </tr>
                        </tfoot>
                    </table>
                ...
                <div class="d-flex justify-content-end">
                    <button type="submit" class="btn btn-primary" id="btn_guardar">Actualizar Orden</button>
                </div>
            </form>
        </div>
    </div>
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        ...
        // Agregar producto existente (al cargar la página)
        function agregarProductoExistente(productoId, nombre, codigo, cantidad, precio, subtotal) {
        ...
        // Agregar producto nuevo a la tabla
